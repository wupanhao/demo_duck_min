...
from duckietown_msgs.srv import GetLanePose,GetLanePoseResponse
...
#         if str(self.detector_config) != str(c):
            ...
            self.detector = LineDetectorHSV(c[1]['configuration'])
...
#         if str(self.detector_config) != str(c):
            ...
            self.detector_intersection = LineDetectorHSV(c[1]['configuration'])
#             self.detector_config = c
    ...
    def processImage_(self, image_msg):
        ...
        seglist_out = self.gp.lineseglist_cb(segmentList)

        lane_pose = self.lane_filter.processSegments(seglist_out)
