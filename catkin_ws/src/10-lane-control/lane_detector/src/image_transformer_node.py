...
class ImageTransformerNode():
    def __init__(self):
        self.node_name = 'image_transformer_node'
        ...
        # Verbose option
        self.verbose = rospy.get_param('image_transformer_verbose', False)

        self.active = rospy.get_param('~image_transformer', True)
        print('image_transformer:',self.active)
        self.locked = False
        ...
        self.pub_raw = rospy.Publisher("/ubiquityrobot/camera_node/image_raw", Image, queue_size=1)
        
        self.pub_image = rospy.Publisher(
            "~corrected_image/compressed", CompressedImage, queue_size=1)

        # self.sub_image = rospy.Subscriber('/ubiquityrobot/camera_node/image_raw', Image, self.callbackImage, queue_size=1)

        if self.active:
            self.sub_image = rospy.Subscriber('/ubiquityrobot/camera_node/image_raw/compressed', CompressedImage, self.callbackImage, queue_size=1)

        self.sub_trafo = rospy.Subscriber(
            '/{}/lane_detector_node/transform'.format(robot_name), AntiInstagramTransform, self.cbNewTrafo, queue_size=1)

        self.sub_colorBalance = rospy.Subscriber(
            '/{}/lane_detector_node/colorBalanceTrafo'.format(robot_name), AntiInstagramTransform_CB, self.cbNewTrafo_CB, queue_size=1)
